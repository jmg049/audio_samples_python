...
class TestSineWaveGeneration:
    ...
    def test_sine_frequency_via_fft(self, sample_rate, medium_duration):
        ...
        fft = audio.rfft(audio.samples_per_channel())
        magnitudes = np.abs(fft)

        # Find peak frequency
        peak_bin = np.argmax(magnitudes)
        freq_resolution = sample_rate / audio.samples_per_channel()
...
class TestNoiseGeneration:
    ...
    def test_white_noise_correct_sample_count(self, sample_rate, short_duration):
        ...
        audio = aus.white_noise(
            duration_secs=short_duration, sample_rate=sample_rate, amplitude=0.5
        )
    ...
    def test_white_noise_matches_numpy_uniform(self, sample_rate, short_duration):
        ...
        audio1 = aus.white_noise(
            duration_secs=short_duration,
            sample_rate=sample_rate,
            amplitude=amplitude,
            seed=1234,
        )
        audio2 = aus.white_noise(
            duration_secs=short_duration,
            sample_rate=sample_rate,
            amplitude=amplitude,
            seed=1234,
        ...
        assert_arrays_close(
            ...
            rtol=1e-15,
            atol=1e-15,
            msg="White noise should be reproducible with same seed",
        )
        ...
        assert (
            abs(np.std(data1) - expected_std) < 0.05
        )  # Std should match theoretical value
    ...
    def test_white_noise_statistics(self, sample_rate, medium_duration):
        ...
        audio = aus.white_noise(
            duration_secs=medium_duration, sample_rate=sample_rate, amplitude=amplitude
        )
...
class TestSquareWaveGeneration:
    ...
    def test_square_wave_matches_scipy(self, sample_rate, short_duration):
        ...
        assert_arrays_close(
            ...
            rtol=1e-4,
            atol=1e-5,
            msg="Square wave does not match scipy.signal.square",
        )
...
class TestSawtoothWaveGeneration:
    ...
    def test_sawtooth_matches_scipy(self, sample_rate, short_duration):
        ...
        assert_arrays_close(
            ...
            rtol=1e-4,
            atol=1e-5,
            msg="Sawtooth wave does not match scipy.signal.sawtooth",
        )
...
#             rtol=1e-4,
#             atol=1e-5,
#             msg="Triangle wave does not match scipy.signal.sawtooth (triangle)",
#         )
...
class TestSilenceGeneration:
    ...
    def test_silence_matches_numpy_zeros(self, sample_rate, short_duration):
        ...
        audio = aus.silence(duration_secs=short_duration, sample_rate=sample_rate)
        data = audio.to_numpy()
        assert_arrays_close(
            data, expected, rtol=0, atol=0, msg="Silence does not match numpy zeros"
        )

    def test_silence_correct_duration(self, sample_rate, medium_duration):
        """Silence has correct duration."""
        audio = aus.silence(duration_secs=medium_duration, sample_rate=sample_rate)
...
class TestDifferentSampleRates:
    ...
    def test_sine_at_various_sample_rates(self, sr):
        ...
        audio = aus.sine_wave(
            frequency=440.0, duration_secs=0.1, sample_rate=sr, amplitude=0.5
        )
    ...
    def test_white_noise_at_various_sample_rates(self, sr):
        ...
        audio = aus.white_noise(duration_secs=0.1, sample_rate=sr, amplitude=0.5)
