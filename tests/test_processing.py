...
class TestScaling:
    ...
    def test_scale_negative_inverts(self, sample_rate):
        ...
        assert_arrays_close(
            aus_audio.to_numpy(), expected, rtol=TIGHT_RTOL, atol=TIGHT_ATOL
        )
...
class TestDCRemoval:
    ...
    def test_remove_dc_offset(self, dc_offset_signal):
        ...
        assert abs(original_mean - dc_offset) < 0.01, (
            "Test signal should have DC offset"
        )
...
class TestReverse:
    ...
    def test_reverse_returns_reversed(self, sample_rate):
        ...
        assert_arrays_close(
            reversed_audio.to_numpy(), expected, rtol=TIGHT_RTOL, atol=TIGHT_ATOL
        )
...
class TestTypeConversion:
    ...
    def test_to_f32_from_f64(self, sine_wave_mono):
        ...
        assert_arrays_close(
            f32_audio.to_numpy(), np_data.astype(np.float32), rtol=1e-5, atol=1e-6
        )
    ...
    def test_to_i16_scaling(self, sample_rate):
        ...
        print(
            f"DEBUG: dtype attribute: {i16_audio.dtype}, type: {type(i16_audio.dtype)}"
        )
    ...
    def test_roundtrip_f64_i16_f64(self, sine_wave_mono):
        ...
        assert_arrays_close(
            back_to_f64.to_numpy(),
            np_data,
            rtol=1e-3,
            atol=1e-4,  # Looser tolerance for quantization
...
class TestArithmeticOperations:
    ...
    def test_add_audio_samples(self, sample_rate):
        ...
        assert_arrays_close(
            result.to_numpy(), expected, rtol=TIGHT_RTOL, atol=TIGHT_ATOL
        )
