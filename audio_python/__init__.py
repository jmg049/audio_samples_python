...
_so_files = list(_package_dir.glob("audio_python.cpython-*.so"))
if _so_files:
    # The module init function is PyInit_audio_python, so we must use "audio_python" as name
    _spec = importlib.util.spec_from_file_location("audio_python", _so_files[0])
    _ext = importlib.util.module_from_spec(_spec)
    # Store the original module in sys.modules temporarily
    _old_module = sys.modules.get("audio_python")
    try:
        _spec.loader.exec_module(_ext)
    finally:
        # Restore ourselves as the audio_python module
        if _old_module is not None:
            sys.modules["audio_python"] = _old_module
